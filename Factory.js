/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║  DRDD DIPE CITY — Factory.js  (v16.14.0)                                     ║
...
window.USE_PLACEHOLDERS = false; // V16.8.0: custom models are the DEFAULT now
  // that all 12 are delivered and wired. Set true to force the procedural
  // builds (useful when testing without an assets/ folder — every character
  // still falls back automatically if its file fails to load).
...
// (characterDipeGenie draft removed V13.7.0 — superseded by the live design below)
  ...
  // an asset.
  // ── V16.3.0: placeholder-box cache ──────────────────────────────────────
  // Building a placeholder just to measure it is cheap but not free, so each
  // measurement is cached. The key includes any NUMERIC argument, because some
  // builders scale themselves from one (the Frogger villains are built at
  // 2.2×–2.8×) and the box differs per value. The throwaway placeholder is
  // never added to a scene, so it allocates no GPU memory and is collected
  // normally — deliberately NOT disposed, since Factory shares materials
  // between builds and disposing them would damage live characters.
  var _phBox = {};
  function _placeholderBox(name, fn, args) {
    var k = name, i;
    for (i = 0; i < args.length; i++) if (typeof args[i] === 'number') k += '|' + args[i];
    if (k in _phBox) return _phBox[k];
    var box = null;
    try {
      var p = fn.apply(null, args);
      if (p && window.Assets && Assets.boxOf) box = Assets.boxOf(p);
    } catch (e) { /* unmeasurable → no fit, model keeps its manifest height */ }
    _phBox[k] = box;
    return box;
  }
    ...
    return function () {
      try {
        if (window.USE_PLACEHOLDERS === false && window.Assets && window.Assets.has(name)) {
          ...
          if (m) {
            // Match the space the placeholder occupied, so spawn heights,
            // collision and camera framing all keep working untouched.
            if (window.GLB_FIT !== false && Assets.fitToBox) {
              var box = _placeholderBox(name, fn, arguments);
              if (box) {
                var spec = (Assets._manifest && Assets._manifest[name]) || null;
                Assets.fitToBox(m, box, spec && spec.fitScale);
              }
            }
            return m;
          }
  ...
  window.Factory = {
    ...
    // ── V22.3.0 — CHINA / FREAKY BLONDE / DUMBASS ─────────────────────────
    // These three existed only as Dipémon sprites; they had no 3D form at all,
    // so there was no Factory key for a GLB to attach to. They now have
    // placeholder builders on the shared humanoid base, which is all the swap
    // pipeline needs: wGLB measures the placeholder's box and fits the real
    // model into exactly that space, so a delivered .glb lands at the right
    // size and height without anything else being touched. Colours only —
    // nobody should be looking at these once the models arrive.
    characterChina:        wGLB('characterChina',        function () { return enemyStooge(0, 0x2b2b36, 0x0a0a12); }),
    characterFreakyBlonde: wGLB('characterFreakyBlonde', function () { return enemyStooge(1, 0x8a44cc, 0x1d0a33); }),
    characterDumbass:      wGLB('characterDumbass',      function () { return enemyStooge(2, 0xd0782c, 0x30170a); }),
