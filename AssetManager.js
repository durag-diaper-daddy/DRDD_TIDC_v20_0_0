...
(function () {
  ...
  function autoUpright(sceneRoot, key) {
    ...
    var before = aspect();
    if (!before || before.upright) return 0;          // already standing
    ...
    if (best === null) {
      ...
      diag('warn', key + ' measures deeper (' + before.d.toFixed(2) + ') than tall (' +
        before.h.toFixed(2) + ') and neither a +90 nor a -90 turn about X stands it up. ' +
        'It is being left as delivered — it probably needs re-exporting upright, or a ' +
        'hand-set rotX in the MANIFEST.');
      return 0;
    }
    diag('log', key + ' was exported lying down — auto-corrected with rotX:' + best +
      ' (height ' + before.h.toFixed(2) + ' → standing). Set rotX explicitly in the ' +
      'MANIFEST if you want a different orientation.');
    return best;
  }
  ...
  A.boot = function () {
    ...
    bootstrapLoader().then(function (boot) {
        ...
        filePromises[file] = new Promise(function (res, rej) {
          (function attempt(n) {
            loader.load(PATH + file, res, undefined, function (err) {
              ...
              if (permanent) {
                diag('warn', file + ' is not on the server (HTTP ' + (status || '404') +
      ...
      loadWave(keys, false);
      if (late.length) setTimeout(function () { loadWave(late, true); }, WAVE2_DELAY);
      function summary() {
