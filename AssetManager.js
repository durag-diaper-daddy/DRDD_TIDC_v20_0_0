...
(function () {
  ...
  var MANIFEST = {
    ...
    // rotX is in DEGREES and is the fix for a model that arrives lying down:
    // an export left in Z-up puts the model's up axis along the viewer's +Z, so
    // it lands flat on its back. -90 stands it up. If any of the other five
    // turn up the same way, add the same line — and if one ends up face-down
    // instead, it wants +90.
    enemyStooge_Moe:         { file: 'stooge_moe.glb',    height: 1.9, rotX: -90, wave: 2 },
    enemyStooge_Larry:       { file: 'stooge_larry.glb',  height: 1.9, wave: 2 },
    enemyStooge_Curly:       { file: 'stooge_curly.glb',  height: 1.9, rotX: -90, wave: 2 },
  ...
  A.boot = function () {
    ...
    bootstrapLoader().then(function (boot) {
      ...
      var pending = 0, lateStarted = false;
      function loadWave(list) {
        pending = list.length;
      ...
      loadWave(keys);
      function summary() {
        if (late.length && !lateStarted) {
          lateStarted = true;
          diag('log', 'first wave done — starting the ' + late.length + ' second-wave model(s)');
          setTimeout(function () { loadWave(late); }, 400);
          return;
        }
